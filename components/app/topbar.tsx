...
export function Topbar() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-40 flex items-center justify-between border-b border-brand-line bg-white/95 px-4 py-3 backdrop-blur dark:border-brand-line dark:bg-[#171f1c]/95 sm:px-8 sm:py-4 lg:left-64`}>
      ...
      <div className="relative flex items-center gap-2 sm:gap-3">
        ...
        {notificationsOpen && <div className="absolute right-0 top-12 z-50 w-[min(360px,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-brand-line bg-white shadow-2xl dark:bg-[#1b2321]"><div className="flex items-center justify-between border-b border-brand-line px-4 py-3"><div><p className="text-sm font-bold text-brand-ink">Notifications</p><p className="text-[10px] text-brand-muted">Realtime market and account activity</p></div><button type="button" onClick={markAllNotificationsRead} className="flex items-center gap-1 text-[10px] font-bold text-brand-limeDeep"><CheckCheck className="h-3.5 w-3.5" />Mark read</button></div><div className="max-h-80 overflow-y-auto">{notifications.length ? notifications.map((item) => <button type="button" key={item.id} onClick={() => markNotificationRead(item.id)} className={`block w-full border-b border-brand-line px-4 py-3 text-left hover:bg-brand-canvas ${item.read ? "opacity-60" : ""}`}><div className="flex items-start justify-between gap-3"><span className="text-xs font-bold text-brand-ink">{item.title}</span>{!item.read && <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-brand-lime" />}</div><p className="mt-1 text-[11px] leading-4 text-brand-muted">{item.body}</p><p className="mt-1 text-[10px] text-brand-muted">{new Date(item.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p></button>) : <div className="px-4 py-10 text-center text-xs text-brand-muted">No notifications yet. Market updates will appear here.</div>}</div></div>}

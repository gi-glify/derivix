...
export function InteractiveCandlestickChart({ points }: { points: PricePoint[] }) {
  ...
  useEffect(() => {
    const handleWheelZoom = (event: WheelEvent) => {
      ...
      setWindowSize((current) => event.deltaY < 0 ? Math.max(12, current - 2) : Math.min(60, current + 2));
    };
  ...
  return <div className="relative h-full min-h-[390px] w-full" data-aos="fade-up">
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-brand-line bg-brand-canvas px-2 py-1.5"><div className="flex min-w-0 flex-wrap items-center gap-1"><span className="px-2 text-[10px] font-bold uppercase tracking-[0.14em] text-brand-muted">Indicators</span><select value={movingAverageWindow ?? "none"} onChange={(event) => setMovingAverageWindow(event.target.value === "none" ? null : Number(event.target.value) as 20 | 50)} className="rounded-lg bg-transparent px-2 py-1.5 text-xs font-bold text-brand-ink"><option value="none">MA off</option><option value="20">MA 20</option><option value="50">MA 50</option></select><button type="button" onClick={() => setAnnotations((value) => !value)} className="flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-bold text-brand-muted hover:bg-brand-line">{annotations ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}Marks</button></div><div className="flex items-center gap-1"><button type="button" onClick={() => setWindowSize((value) => Math.min(60, value + 4))} className="rounded-lg p-1.5 text-brand-muted hover:bg-brand-line" aria-label="Zoom out"><Minus className="h-3.5 w-3.5" /></button><button type="button" onClick={() => setWindowSize((value) => Math.max(12, value - 4))} className="rounded-lg p-1.5 text-brand-muted hover:bg-brand-line" aria-label="Zoom in"><Plus className="h-3.5 w-3.5" /></button><button type="button" onClick={() => { setZoomStart(0); setFollowingLatest(true); }} className="rounded-lg p-1.5 text-brand-muted hover:bg-brand-line" aria-label="Reset chart zoom"><RotateCcw className="h-3.5 w-3.5" /></button><button type="button" onClick={downloadData} className="rounded-lg p-1.5 text-brand-muted hover:bg-brand-line" aria-label="Download chart data"><Download className="h-3.5 w-3.5" /></button></div></div>

...
export function InteractiveCandlestickChart({ points }: { points: PricePoint[] }) {
  ...
  const handleMove = (event: MouseEvent<SVGSVGElement>) => {
    ...
    const rawIndex = Math.floor((chartX - PLOT.left) / step);
    const index = Math.max(0, Math.min(windowPoints.length - 1, rawIndex));
    if (chartX < PLOT.left || chartX > PLOT.right || !windowPoints[index]) { setHover(null); return; }
    setHover({ index, x: PLOT.left + index * step + step / 2, y: scaleY(windowPoints[index].close, min, span) });
  };

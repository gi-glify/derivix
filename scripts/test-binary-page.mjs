...
const indices=[
 {symbol:'V10',name:'Volatility 10 Index',precision:2,base_price:'267860.19',tick_interval_ms:2000,movement_scale:'0.1'},
 {symbol:'V50_1S',name:'Volatility 50 (1s) Index',precision:2,base_price:'267860.19',tick_interval_ms:1000,movement_scale:'0.5'},
];
...
await context.route('**/*',async route=>{
 ...
 if(u.pathname.includes('/auth/v1/user')) result=user;
 else if(u.pathname.endsWith('/rpc/binary_state')) {
   ...
   stateCalls++; result={contracts,ticks:mode==='empty'?{}:{V10:ticks,V50_1S:ticks}};
 }
 else if(u.pathname.endsWith('/rpc/binary_advance_ticks')) { tickCalls++; result={recorded:2}; }
...
assert.equal(await page.getByLabel('Index',{exact:true}).inputValue(),'V10');
assert.equal(await page.locator('optgroup').count(),2, 'standard and 1-second indices are grouped');
...
await page.getByLabel('Index',{exact:true}).selectOption('V50_1S');
await page.locator('.binary-chart .chart-canvas').waitFor();
